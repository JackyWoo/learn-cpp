...
inline uint64_t clock_gettime_ns(clockid_t clock_type = STOPWATCH_DEFAULT_CLOCK)
{
    struct timespec ts;
...
inline uint64_t clock_gettime_ns_adjusted(uint64_t prev_time, clockid_t clock_type = STOPWATCH_DEFAULT_CLOCK)
{
#ifdef CLOCK_MONOTONIC_RAW
    if (clock_type == CLOCK_MONOTONIC_RAW)
...
public:
    ...
    uint64_t getStart() const { return start_ns; }
    uint64_t getEnd() const { return stop_ns; }
...
private:
    ...
    uint64_t nanoseconds(uint64_t prev_time) const { return clock_gettime_ns_adjusted(prev_time, clock_type) & 0x7FFFFFFFFFFFFFFFULL; }
};
