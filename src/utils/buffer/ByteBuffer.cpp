...
/**
 * ByteBuffer constructor
 * Consume an entire char array of length len in the ByteBuffer
 *
 * @param arr char array of data (should be of length len)
 * @param size Size of space to allocate
 */
    ByteBuffer::ByteBuffer(char* arr, uint32_t size) {
...
/**
 * Replace
 * Replace occurance of a particular char, key, with the char rep
 *
 * @param key char to find for replacement
 * @param rep char to replace the found key with
 ...
 */
    void ByteBuffer::replace(char key, char rep, uint32_t start, bool firstOccuranceOnly) {
        uint32_t len = buf.size();
        for (uint32_t i = start; i < len; i++) {
            char data = read<char>(i);
...
// Read Functions
    ...
    char ByteBuffer::get(uint32_t index) const {
        return read<char>(index);
    }

    void ByteBuffer::getBytes(char* buf_, uint32_t len) const {
        for (uint32_t i = 0; i < len; i++) {
            buf_[i] = read<char>();
        }
    }
...
// Write Functions
    ...
    void ByteBuffer::put(char b, uint32_t index) {
        insert<char>(b, index);
    }

    void ByteBuffer::putBytes(const char* b, uint32_t len) {
        // Insert the data one byte at a time into the internal buffer at position i+starting index
        for (uint32_t i = 0; i < len; i++)
            append<char>(b[i]);
    }

    void ByteBuffer::putBytes(const char* b, uint32_t len, uint32_t index) {
        wpos = index;
