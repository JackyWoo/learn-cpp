 ...
 */
template <typename T>
class ConcurrentBoundedSingleThreadQueue
{
public:
    explicit ConcurrentBoundedSingleThreadQueue(size_t capacity = 10000);
...
template <typename T>
ConcurrentBoundedSingleThreadQueue<T>::ConcurrentBoundedSingleThreadQueue(size_t capacity_)
    : buffer(capacity_), head(0), tail(0), count(0), capacity(capacity_)
{
}

template <typename T>
void ConcurrentBoundedSingleThreadQueue<T>::push(const T & item)
{
    if (stopped.load())
        return;
...
template <typename T>
bool ConcurrentBoundedSingleThreadQueue<T>::tryPush(const T & item, uint64_t timeout_ms)
{
...
template <typename T>
void ConcurrentBoundedSingleThreadQueue<T>::pop(T & item)
{
    if (stopped.load())
        return;
...
template <typename T>
bool ConcurrentBoundedSingleThreadQueue<T>::tryPop(T & item, uint64_t timeout_ms)
{
    using namespace std::chrono;
...
template <typename T>
size_t ConcurrentBoundedSingleThreadQueue<T>::size() const
{
    return count;
}

template <typename T>
bool ConcurrentBoundedSingleThreadQueue<T>::empty() const
{
    return count == 0;
}

template <typename T>
void ConcurrentBoundedSingleThreadQueue<T>::stop()
{
    stopped.store(true);
}
