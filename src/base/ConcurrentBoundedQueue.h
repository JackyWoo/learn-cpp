...
private:
    ...
    bool emplaceImpl(std::optional<uint64_t> timeout_milliseconds, Args &&...args)
    {
        {
    ...
    template <bool front>
    bool popImpl(T & x, std::optional<uint64_t> timeout_milliseconds)
    {
        {
            std::unique_lock<std::mutex> queue_lock(queue_mutex);
...
public:
    ...
    [[nodiscard]] bool tryPush(const T & x, uint64_t milliseconds = 0)
    {
        return emplaceImpl</* back= */ true>(milliseconds, x);
    }

    [[nodiscard]] bool tryPush(T && x, uint64_t milliseconds = 0)
    {
    ...
    template <typename... Args>
    [[nodiscard]] bool tryEmplace(uint64_t milliseconds, Args &&... args)
    {
    ...
    [[nodiscard]] bool tryPop(T & x, uint64_t milliseconds)
    {
